...
describe 'prometheus haproxy_exporter' do
  it 'haproxy_exporter works idempotently with no errors' do
    pp = 'include prometheus::exporter::haproxy'
  ...
  describe 'haproxy_exporter update from 0.7.1 to 0.9.0' do
    it 'is idempotent' do
      pp = "class{'prometheus::exporter::haproxy': version => '0.7.1'}"
      # Run it twice and test for idempotency
      apply_manifest(pp, catch_failures: true)
      apply_manifest(pp, catch_changes: true)
    end
    ...
    it 'is idempotent' do
      pp = "class{'prometheus::exporter::haproxy': version => '0.9.0'}"
      # Run it twice and test for idempotency
      apply_manifest(pp, catch_failures: true)
      apply_manifest(pp, catch_changes: true)
    end
  ...
  describe 'haproxy_exporter update from 0.9.0 to 0.10.0' do
    ...
    it 'is idempotent' do
      pp = "class{'prometheus::exporter::haproxy': version => '0.10.0'}"
      # Run it twice and test for idempotency
      apply_manifest(pp, catch_failures: true)
      apply_manifest(pp, catch_changes: true)
    end
