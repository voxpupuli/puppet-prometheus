...
#
# This module manages prometheus haproxy haproxy_exporter
#
# Parameters:
#  [*arch*]
...
class prometheus::haproxy_exporter (
  ...
  $scrape_uri           = $::prometheus::params::haproxy_exporter_scrape_uri,
  ...
  # Prometheus added a 'v' on the realease name at 0.7.1
  if versioncmp ($version, '0.7.1') >= 0 {
    $release = "v${version}"
  }
  else {
    $release = $version
  ...
  $options = "-haproxy.scrape-uri=${scrape_uri} ${extra_options}"
